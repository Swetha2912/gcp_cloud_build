...
func main() {
	...
	// set default s3 Credentials for model upload
	defaultCredentialID = os.Getenv("DEFAULT_CREDENTIAL_ID")
	if defaultCredentialID == "" {
		fmt.Println("Log -- please add default CredentialID exiting ..")
		os.Exit(3)
	}
	defaultBucketName = os.Getenv("DEFAULT_BUCKET_NAME")
	if defaultBucketName == "" {
		fmt.Println("Log -- please add default Bucket name exiting ..")
		os.Exit(3)
	}
	...
	//MongoDB connection
	modelSession, err := mgo.Dial("mongodb://" + os.Getenv("MODEL_DB_HOST"))
	if modelSession != nil {
		collections["Model"] = modelSession.DB(os.Getenv("MODEL_DB_NAME")).C("models")
		modelSession.SetSocketTimeout(1 * time.Second)
	} else {
